...
io.on('connection', function(socket) {
	...
	socket.on("getMessages",function(username){
		...
		for(i=0;i<=messages.length-1;i++){
			...
			if(!messages[i].deleted){
				io.sockets.emit("getMessage",{to:messages[i].to,from:messages[i].from,subject:messages[i].subject,message:messages[i].message,date:messages[i].date,id:messages[i].id});
			}
	...
	socket.on("sendMessage",function(data){
		...
		if(userExists){
			...
			io.sockets.emit("getMessage",{to:data.to,from:data.from,subject:data.subject,message:data.message,date:data.date,id:messageId});
			messageId++;
		}
